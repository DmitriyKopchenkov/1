...
public class Port {
    ...
    public Set<List<Integer>> getUniqueGroupsElements() {
        ...
        recursive(0, new ArrayList<>(), length, result);
        return result;
    }


    private void recursive(int position, List<Integer> constructor, int length, Set<List<Integer>> result) {
        if (position == length) {
            ...
            for (Integer number : array) {
                ...
                list.add(number);
                recursive(position + 1, list, length, result);
            }
        }
    }
